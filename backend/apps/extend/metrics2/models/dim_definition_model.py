...
class DimDict(SQLModel, table=True):
    ...
    id: str = Field(sa_column=Column(VARCHAR(32), primary_key=True, comment='维度唯一ID，格式：D+6位数字（如D000001），自增'))
    name: str = Field(sa_column=Column(VARCHAR(64), nullable=False, comment='维度中文名称（如猪场编号、胎次号）'))
